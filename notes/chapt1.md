...
Sunday算法同Horspool算法思想相同，不过Sunday算法使用的字符是比较窗口后面的一个字符，从而增加了窗口移动的距离。
...
BDM算法使用了**后缀自动机**的方式进行子字符串的查找，而它BNDM算法的关系就像KMP算法与Shift-And算法的关系一样，BNDM算法使用了位并行的方法增加了BDM算法的效率。下图是BDM算法所用的后缀自动机的一个实例。

![](images/1/2.png)

对于这部分这个帖子——[打破思维断层之最优美的BNDM](http://wlh0706-163-com.iteye.com/blog/1858230)，有着很深刻的理解。

### 3.2 BOM算法

在pattern的长度超过1个机器字长的时候，如果使用BNDM来构造mask就会非常麻烦，所以有人提出了BOM算法。BOM算法使用了Factor Oracle（一个确定性有向无环自动机）来判断一个content的后缀是否是pattern的一个子串，如果发现当前读入的字符与之前读入的content的后缀组合起来不是pattern的一个子串，那么就可以安全的移动窗口。

Factor Oracle可以在遍历pattern的同时在O(m)的时间复杂度内构造，具体见[代码](../src/chapt1/BOM.java)中的注释，一个例子如下图所示：
